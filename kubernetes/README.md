...
```
nodejs-app pod  ──► port 3306 ──►  mysql pod   ✅ ALLOWED
other pods      ──► port 3306 ──►  mysql pod   🚫 BLOCKED
```
...
metadata:
  ...
  namespace: ivolve
spec:
  podSelector:          # targets MySQL pods
    matchLabels:
      app: mysql
  policyTypes:
  - Ingress             # only controls incoming traffic to MySQL
  ingress:
  - from:
    - podSelector:      # only allow from nodejs-app pods
        matchLabels:
          app: nodejs-app
    ports:
    - protocol: TCP
...
spec:
  ...
  template:
    ...
    spec:
      tolerations:
      - key: "node"               # tolerate the worker node taint
        operator: "Equal"
        value: "worker"
        effect: "NoSchedule"
      - operator: "Exists"        # tolerate any other future taints
        effect: "NoSchedule"

      hostNetwork: true           # use node's network directly
...
node-exporter-yyyyy     1/1     Running   minikube-m02
```
